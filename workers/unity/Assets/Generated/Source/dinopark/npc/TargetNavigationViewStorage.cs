...
namespace Dinopark.Npc
{
    ...
    {
        public class TargetNavigationViewStorage : IViewStorage, IViewComponentStorage<Snapshot>,
            IViewComponentUpdater<Update>
        {
            ...
            public void ApplyUpdate(long entityId, in Update update)
            {
                ...
                {
                    data.NavigationState = update.NavigationState.Value;
                }

                if (update.TargetPosition.HasValue)
                {
                    data.TargetPosition = update.TargetPosition.Value;
                }

                if (update.TargetEntityId.HasValue)
                {
                    data.TargetEntityId = update.TargetEntityId.Value;
                }

                if (update.InteractionSqrDistance.HasValue)
                {
                    data.InteractionSqrDistance = update.InteractionSqrDistance.Value;
                }
