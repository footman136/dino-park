...
namespace Dinopark.Npc
{
    ...
    public class NPCLumberjackReaderSubscriptionManager : SubscriptionManager<NPCLumberjackReader>
    {
        private readonly EntityManager entityManager;
        private readonly World world;
        private readonly WorkerSystem workerSystem;

        private Dictionary<EntityId, HashSet<Subscription<NPCLumberjackReader>>> entityIdToReaderSubscriptions;
        ...
        public NPCLumberjackReaderSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterComponentAddedCallback(NPCLumberjack.ComponentId, entityId =>
            {
                ...
                {
                    subscription.SetAvailable(new NPCLumberjackReader(world, entity, entityId));
                }
            ...
            constraintCallbackSystem.RegisterComponentRemovedCallback(NPCLumberjack.ComponentId, entityId =>
            {
        ...
        public override Subscription<NPCLumberjackReader> Subscribe(EntityId entityId)
        {
            if (entityIdToReaderSubscriptions == null)
            {
                entityIdToReaderSubscriptions = new Dictionary<EntityId, HashSet<Subscription<NPCLumberjackReader>>>();
            }

            var subscription = new Subscription<NPCLumberjackReader>(this, entityId);

            if (!entityIdToReaderSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<NPCLumberjackReader>>();
                entityIdToReaderSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && entityManager.HasComponent<NPCLumberjack.Component>(entity))
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new NPCLumberjackReader(world, entity, entityId));
            }
        ...
        {
            var sub = ((Subscription<NPCLumberjackReader>) subscription);
            if (sub.HasValue)
            {
    ...
    [AutoRegisterSubscriptionManager]
    public class NPCLumberjackWriterSubscriptionManager : SubscriptionManager<NPCLumberjackWriter>
    {
        private readonly World world;
        private readonly WorkerSystem workerSystem;
        private readonly ComponentUpdateSystem componentUpdateSystem;

        private Dictionary<EntityId, HashSet<Subscription<NPCLumberjackWriter>>> entityIdToWriterSubscriptions;

        private HashSet<EntityId> entitiesMatchingRequirements = new HashSet<EntityId>();
        private HashSet<EntityId> entitiesNotMatchingRequirements = new HashSet<EntityId>();

        public NPCLumberjackWriterSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterAuthorityCallback(NPCLumberjack.ComponentId, authorityChange =>
            {
                if (authorityChange.Authority == Authority.Authoritative)
                {
                    ...
                    {
                        subscription.SetAvailable(new NPCLumberjackWriter(world, entity, authorityChange.EntityId));
                    }
        ...
        public override Subscription<NPCLumberjackWriter> Subscribe(EntityId entityId)
        {
            if (entityIdToWriterSubscriptions == null)
            {
                entityIdToWriterSubscriptions = new Dictionary<EntityId, HashSet<Subscription<NPCLumberjackWriter>>>();
            }

            var subscription = new Subscription<NPCLumberjackWriter>(this, entityId);

            if (!entityIdToWriterSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<NPCLumberjackWriter>>();
                entityIdToWriterSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && componentUpdateSystem.HasComponent(NPCLumberjack.ComponentId, entityId)
                && componentUpdateSystem.GetAuthority(entityId, NPCLumberjack.ComponentId) != Authority.NotAuthoritative)
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new NPCLumberjackWriter(world, entity, entityId));
            }
            else
            {
                entitiesNotMatchingRequirements.Add(entityId);
        ...
        public override void Cancel(ISubscription subscription)
        {
            var sub = ((Subscription<NPCLumberjackWriter>) subscription);
            if (sub.HasValue)
            {
                var reader = sub.Value;
                reader.IsValid = false;
    ...
    public class NPCLumberjackReader
    {
        public bool IsValid;
        ...
        public NPCLumberjack.Component Data
        {
            get
            {
                if (!IsValid)
                ...
                return EntityManager.GetComponentData<NPCLumberjack.Component>(Entity);
            }
        }
        ...
        public Authority Authority
        {
            get
            {
                ...
                return ComponentUpdateSystem.GetAuthority(EntityId, NPCLumberjack.ComponentId);
            }
        }
        ...
        public event Action<Authority> OnAuthorityUpdate
        {
            add
            {
                ...
                var key = CallbackSystem.RegisterAuthorityCallback(EntityId, NPCLumberjack.ComponentId, value);
        ...
        private Dictionary<Action<NPCLumberjack.Update>, ulong> updateCallbackToCallbackKey;
        public event Action<NPCLumberjack.Update> OnUpdate
        {
            add
            {
                if (updateCallbackToCallbackKey == null)
                {
                    updateCallbackToCallbackKey = new Dictionary<Action<NPCLumberjack.Update>, ulong>();
                }
        ...
        public event Action<global::Dinopark.Npc.LumberjackFSMState.StateEnum> OnCurrentStateUpdate
        {
            add
            {
                ...
                var key = CallbackSystem.RegisterComponentUpdateCallback<NPCLumberjack.Update>(EntityId, update =>
                {
        ...
        internal NPCLumberjackReader(World world, Entity entity, EntityId entityId)
        {
            Entity = entity;
            EntityId = entityId;
    ...
    public class NPCLumberjackWriter : NPCLumberjackReader
    {
        internal NPCLumberjackWriter(World world, Entity entity, EntityId entityId)
            : base(world, entity, entityId)
        {
        }

        public void SendUpdate(NPCLumberjack.Update update)
        {
            var component = EntityManager.GetComponentData<NPCLumberjack.Component>(Entity);
        ...
        {
            ComponentUpdateSystem.AcknowledgeAuthorityLoss(EntityId, NPCLumberjack.ComponentId);
        }
    }
}
