...
namespace Dinopark.Npc
{
    public partial class TargetNavigation
    {
        public class DiffComponentStorage : IDiffUpdateStorage<Update>, IDiffComponentAddedStorage<Update>, IDiffAuthorityStorage
            , IDiffEventStorage<NavigationFinished.Event>
        {
            ...
            private MessageList<ComponentEventReceived<NavigationFinished.Event>> navigationFinishedEventStorage =
                new MessageList<ComponentEventReceived<NavigationFinished.Event>>();

            private readonly EventComparer<NavigationFinished.Event> navigationFinishedComparer =
                new EventComparer<NavigationFinished.Event>();

            public Type[] GetEventTypes()
            {
                return new Type[]
                {
                    typeof(NavigationFinished.Event),
                };
            }
            ...
            public void Clear()
            {
                ...
                navigationFinishedEventStorage.Clear();
            }
            ...
            public void RemoveEntityComponent(long entityId)
            {
                ...
                if (entitiesUpdated.Remove(id))
                {
                    ...
                    navigationFinishedEventStorage.RemoveAll(change => change.EntityId.Id == entityId);
                }
            ...
            ReceivedMessagesSpan<ComponentEventReceived<NavigationFinished.Event>> IDiffEventStorage<NavigationFinished.Event>.GetEvents(EntityId entityId)
            {
                var range = navigationFinishedEventStorage.GetEntityRange(entityId);
                return new ReceivedMessagesSpan<ComponentEventReceived<NavigationFinished.Event>>(
                    navigationFinishedEventStorage, range.FirstIndex, range.Count);
            }

            ReceivedMessagesSpan<ComponentEventReceived<NavigationFinished.Event>> IDiffEventStorage<NavigationFinished.Event>.GetEvents()
            {
                return new ReceivedMessagesSpan<ComponentEventReceived<NavigationFinished.Event>>(navigationFinishedEventStorage);
            }

            void IDiffEventStorage<NavigationFinished.Event>.AddEvent(ComponentEventReceived<NavigationFinished.Event> ev)
            {
                navigationFinishedEventStorage.InsertSorted(ev, navigationFinishedComparer);
            }
        }
    }
}
