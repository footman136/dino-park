...
namespace Dinopark.Core
{
    public partial class TransformComponent
    {
        public class DiffComponentStorage : IDiffUpdateStorage<Update>, IDiffComponentAddedStorage<Update>, IDiffAuthorityStorage
            , IDiffEventStorage<TeleportEvent.Event>
        {
            ...
            private MessageList<ComponentEventReceived<TeleportEvent.Event>> teleportEventEventStorage =
                new MessageList<ComponentEventReceived<TeleportEvent.Event>>();

            private readonly EventComparer<TeleportEvent.Event> teleportEventComparer =
                new EventComparer<TeleportEvent.Event>();

            public Type[] GetEventTypes()
            {
                return new Type[]
                {
                    typeof(TeleportEvent.Event),
                };
            }
            ...
            public void Clear()
            {
                ...
                teleportEventEventStorage.Clear();
            }
            ...
            public void RemoveEntityComponent(long entityId)
            {
                ...
                if (entitiesUpdated.Remove(id))
                {
                    ...
                    teleportEventEventStorage.RemoveAll(change => change.EntityId.Id == entityId);
                }
            ...
            ReceivedMessagesSpan<ComponentEventReceived<TeleportEvent.Event>> IDiffEventStorage<TeleportEvent.Event>.GetEvents(EntityId entityId)
            {
                var range = teleportEventEventStorage.GetEntityRange(entityId);
                return new ReceivedMessagesSpan<ComponentEventReceived<TeleportEvent.Event>>(
                    teleportEventEventStorage, range.FirstIndex, range.Count);
            }

            ReceivedMessagesSpan<ComponentEventReceived<TeleportEvent.Event>> IDiffEventStorage<TeleportEvent.Event>.GetEvents()
            {
                return new ReceivedMessagesSpan<ComponentEventReceived<TeleportEvent.Event>>(teleportEventEventStorage);
            }

            void IDiffEventStorage<TeleportEvent.Event>.AddEvent(ComponentEventReceived<TeleportEvent.Event> ev)
            {
                teleportEventEventStorage.InsertSorted(ev, teleportEventComparer);
            }
        }
    }
}
