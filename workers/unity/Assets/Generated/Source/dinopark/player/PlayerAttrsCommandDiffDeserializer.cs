...
namespace Dinopark.Player
{
    ...
    {
        public class LayEggDiffCommandDeserializer : ICommandDiffDeserializer
        {
            ...
            {
                var deserializedRequest = global::Dinopark.Player.LayEggRequest.Serialization.Deserialize(op.Request.SchemaData.Value.GetObject());

                var request = new LayEgg.ReceivedRequest(
            ...
            {
                global::Dinopark.Player.LayEggResponse? rawResponse = null;
                if (op.StatusCode == StatusCode.Success)
                {
                    rawResponse = global::Dinopark.Player.LayEggResponse.Serialization.Deserialize(op.Response.SchemaData.Value.GetObject());
                }

                var commandContext = commandMetaData.GetContext<global::Dinopark.Player.LayEggRequest>(ComponentId, 1, op.RequestId);
                commandMetaData.MarkIdForRemoval(ComponentId, 1, op.RequestId);

                var response = new LayEgg.ReceivedResponse(
        ...
        public class LayEggCommandSerializer : ICommandSerializer
        {
            public uint GetComponentId()
            {
                return ComponentId;
            ...
            {
                var storage = (LayEggCommandsToSendStorage) messages.GetCommandSendStorage(ComponentId, 1);
                ...
                for (int i = 0; i < requests.Count; ++i)
                {
                    ...
                    var context = new CommandContext<global::Dinopark.Player.LayEggRequest>(request.SendingEntity, request.Request.Payload, request.Request.Context, request.RequestId);
                    commandMetaData.AddRequest<global::Dinopark.Player.LayEggRequest>(ComponentId, 1, in context);

                    var schemaCommandRequest = new global::Improbable.Worker.CInterop.SchemaCommandRequest(ComponentId, 1);
                    global::Dinopark.Player.LayEggRequest.Serialization.Serialize(request.Request.Payload, schemaCommandRequest.GetObject());
                ...
                for (int i = 0; i < responses.Count; ++i)
                {
                    ...
                    global::Dinopark.Player.LayEggResponse.Serialization.Serialize(response.Payload.Value, schemaCommandResponse.GetObject());
