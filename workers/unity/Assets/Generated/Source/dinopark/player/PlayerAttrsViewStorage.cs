...
namespace Dinopark.Player
{
    ...
    {
        public class PlayerAttrsViewStorage : IViewStorage, IViewComponentStorage<Snapshot>,
            IViewComponentUpdater<Update>
        {
            ...
            public void ApplyUpdate(long entityId, in Update update)
            {
                ...
                {
                    data.Level = update.Level.Value;
                }

                if (update.Exp.HasValue)
                {
                    data.Exp = update.Exp.Value;
                }

                if (update.Energy.HasValue)
                {
                    data.Energy = update.Energy.Value;
                }
