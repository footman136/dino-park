    ...
    public partial class PlayerAttrs
    {
        public class EcsViewManager : IEcsViewManager
        {
            ...
            private void RemoveComponent(EntityId entityId)
            {
                ...
                entityManager.RemoveComponent<global::Dinopark.Player.PlayerAttrs.Component>(entity);
            }
            ...
            private void ApplyUpdate(in ComponentUpdateReceived<Update> update, ComponentDataFromEntity<Component> dataFromEntity)
            {
                ...
                if (update.Update.Level.HasValue)
                {
                    data.Level = update.Update.Level.Value;
                }

                if (update.Update.Exp.HasValue)
                {
                    data.Exp = update.Update.Exp.Value;
                }

                if (update.Update.Energy.HasValue)
                {
                    data.Energy = update.Update.Energy.Value;
                }
