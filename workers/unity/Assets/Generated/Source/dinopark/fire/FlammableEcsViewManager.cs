    ...
    public partial class Flammable
    {
        public class EcsViewManager : IEcsViewManager
        {
            ...
            private void RemoveComponent(EntityId entityId)
            {
                ...
                entityManager.RemoveComponent<global::Dinopark.Fire.Flammable.Component>(entity);
            }
            ...
            private void ApplyUpdate(in ComponentUpdateReceived<Update> update, ComponentDataFromEntity<Component> dataFromEntity)
            {
                ...
                if (update.Update.IsOnFire.HasValue)
                {
                    data.IsOnFire = update.Update.IsOnFire.Value;
                }

                if (update.Update.CanBeIgnited.HasValue)
                {
                    data.CanBeIgnited = update.Update.CanBeIgnited.Value;
                }

                if (update.Update.EffectType.HasValue)
                {
                    data.EffectType = update.Update.EffectType.Value;
                }
