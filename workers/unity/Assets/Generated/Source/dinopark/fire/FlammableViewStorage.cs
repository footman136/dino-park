...
namespace Dinopark.Fire
{
    ...
    {
        public class FlammableViewStorage : IViewStorage, IViewComponentStorage<Snapshot>,
            IViewComponentUpdater<Update>
        {
            ...
            public void ApplyUpdate(long entityId, in Update update)
            {
                ...
                {
                    data.IsOnFire = update.IsOnFire.Value;
                }

                if (update.CanBeIgnited.HasValue)
                {
                    data.CanBeIgnited = update.CanBeIgnited.Value;
                }

                if (update.EffectType.HasValue)
                {
                    data.EffectType = update.EffectType.Value;
                }
