...
namespace Improbable.Restricted
{
    ...
    public class SystemReaderSubscriptionManager : SubscriptionManager<SystemReader>
    {
        private readonly EntityManager entityManager;
        private readonly World world;
        private readonly WorkerSystem workerSystem;

        private Dictionary<EntityId, HashSet<Subscription<SystemReader>>> entityIdToReaderSubscriptions;
        ...
        public SystemReaderSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterComponentAddedCallback(System.ComponentId, entityId =>
            {
                ...
                {
                    subscription.SetAvailable(new SystemReader(world, entity, entityId));
                }
            ...
            constraintCallbackSystem.RegisterComponentRemovedCallback(System.ComponentId, entityId =>
            {
        ...
        public override Subscription<SystemReader> Subscribe(EntityId entityId)
        {
            if (entityIdToReaderSubscriptions == null)
            {
                entityIdToReaderSubscriptions = new Dictionary<EntityId, HashSet<Subscription<SystemReader>>>();
            }

            var subscription = new Subscription<SystemReader>(this, entityId);

            if (!entityIdToReaderSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<SystemReader>>();
                entityIdToReaderSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && entityManager.HasComponent<System.Component>(entity))
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new SystemReader(world, entity, entityId));
            }
        ...
        {
            var sub = ((Subscription<SystemReader>) subscription);
            if (sub.HasValue)
            {
    ...
    [AutoRegisterSubscriptionManager]
    public class SystemWriterSubscriptionManager : SubscriptionManager<SystemWriter>
    {
        private readonly World world;
        private readonly WorkerSystem workerSystem;
        private readonly ComponentUpdateSystem componentUpdateSystem;

        private Dictionary<EntityId, HashSet<Subscription<SystemWriter>>> entityIdToWriterSubscriptions;

        private HashSet<EntityId> entitiesMatchingRequirements = new HashSet<EntityId>();
        private HashSet<EntityId> entitiesNotMatchingRequirements = new HashSet<EntityId>();

        public SystemWriterSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterAuthorityCallback(System.ComponentId, authorityChange =>
            {
                if (authorityChange.Authority == Authority.Authoritative)
                {
                    ...
                    {
                        subscription.SetAvailable(new SystemWriter(world, entity, authorityChange.EntityId));
                    }
        ...
        public override Subscription<SystemWriter> Subscribe(EntityId entityId)
        {
            if (entityIdToWriterSubscriptions == null)
            {
                entityIdToWriterSubscriptions = new Dictionary<EntityId, HashSet<Subscription<SystemWriter>>>();
            }

            var subscription = new Subscription<SystemWriter>(this, entityId);

            if (!entityIdToWriterSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<SystemWriter>>();
                entityIdToWriterSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && componentUpdateSystem.HasComponent(System.ComponentId, entityId)
                && componentUpdateSystem.GetAuthority(entityId, System.ComponentId) != Authority.NotAuthoritative)
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new SystemWriter(world, entity, entityId));
            }
            else
            {
                entitiesNotMatchingRequirements.Add(entityId);
        ...
        public override void Cancel(ISubscription subscription)
        {
            var sub = ((Subscription<SystemWriter>) subscription);
            if (sub.HasValue)
            {
                var reader = sub.Value;
                reader.IsValid = false;
    ...
    public class SystemReader
    {
        ...
        public System.Component Data
        {
            get
            {
                ...
                return EntityManager.GetComponentData<System.Component>(Entity);
            }
        }
        ...
        public Authority Authority
        {
            get
            {
                ...
                return ComponentUpdateSystem.GetAuthority(EntityId, System.ComponentId);
            }
        }
        ...
        public event Action<Authority> OnAuthorityUpdate
        {
            add
            {
                ...
                var key = CallbackSystem.RegisterAuthorityCallback(EntityId, System.ComponentId, value);
        ...
        private Dictionary<Action<System.Update>, ulong> updateCallbackToCallbackKey;
        public event Action<System.Update> OnUpdate
        {
            add
            {
                if (updateCallbackToCallbackKey == null)
                {
                    updateCallbackToCallbackKey = new Dictionary<Action<System.Update>, ulong>();
                }
        ...
        internal SystemReader(World world, Entity entity, EntityId entityId)
        {
            Entity = entity;
            EntityId = entityId;
