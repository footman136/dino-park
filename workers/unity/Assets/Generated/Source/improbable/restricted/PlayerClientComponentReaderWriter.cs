...
namespace Improbable.Restricted
{
    ...
    public class PlayerClientReaderSubscriptionManager : SubscriptionManager<PlayerClientReader>
    {
        private readonly EntityManager entityManager;
        private readonly World world;
        private readonly WorkerSystem workerSystem;

        private Dictionary<EntityId, HashSet<Subscription<PlayerClientReader>>> entityIdToReaderSubscriptions;
        ...
        public PlayerClientReaderSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterComponentAddedCallback(PlayerClient.ComponentId, entityId =>
            {
                ...
                {
                    subscription.SetAvailable(new PlayerClientReader(world, entity, entityId));
                }
            ...
            constraintCallbackSystem.RegisterComponentRemovedCallback(PlayerClient.ComponentId, entityId =>
            {
        ...
        public override Subscription<PlayerClientReader> Subscribe(EntityId entityId)
        {
            if (entityIdToReaderSubscriptions == null)
            {
                entityIdToReaderSubscriptions = new Dictionary<EntityId, HashSet<Subscription<PlayerClientReader>>>();
            }

            var subscription = new Subscription<PlayerClientReader>(this, entityId);

            if (!entityIdToReaderSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<PlayerClientReader>>();
                entityIdToReaderSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && entityManager.HasComponent<PlayerClient.Component>(entity))
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new PlayerClientReader(world, entity, entityId));
            }
        ...
        {
            var sub = ((Subscription<PlayerClientReader>) subscription);
            if (sub.HasValue)
            {
    ...
    [AutoRegisterSubscriptionManager]
    public class PlayerClientWriterSubscriptionManager : SubscriptionManager<PlayerClientWriter>
    {
        private readonly World world;
        private readonly WorkerSystem workerSystem;
        private readonly ComponentUpdateSystem componentUpdateSystem;

        private Dictionary<EntityId, HashSet<Subscription<PlayerClientWriter>>> entityIdToWriterSubscriptions;

        private HashSet<EntityId> entitiesMatchingRequirements = new HashSet<EntityId>();
        private HashSet<EntityId> entitiesNotMatchingRequirements = new HashSet<EntityId>();

        public PlayerClientWriterSubscriptionManager(World world)
        {
            this.world = world;
            ...
            constraintCallbackSystem.RegisterAuthorityCallback(PlayerClient.ComponentId, authorityChange =>
            {
                if (authorityChange.Authority == Authority.Authoritative)
                {
                    ...
                    {
                        subscription.SetAvailable(new PlayerClientWriter(world, entity, authorityChange.EntityId));
                    }
        ...
        public override Subscription<PlayerClientWriter> Subscribe(EntityId entityId)
        {
            if (entityIdToWriterSubscriptions == null)
            {
                entityIdToWriterSubscriptions = new Dictionary<EntityId, HashSet<Subscription<PlayerClientWriter>>>();
            }

            var subscription = new Subscription<PlayerClientWriter>(this, entityId);

            if (!entityIdToWriterSubscriptions.TryGetValue(entityId, out var subscriptions))
            {
                subscriptions = new HashSet<Subscription<PlayerClientWriter>>();
                entityIdToWriterSubscriptions.Add(entityId, subscriptions);
            }

            if (workerSystem.TryGetEntity(entityId, out var entity)
                && componentUpdateSystem.HasComponent(PlayerClient.ComponentId, entityId)
                && componentUpdateSystem.GetAuthority(entityId, PlayerClient.ComponentId) != Authority.NotAuthoritative)
            {
                entitiesMatchingRequirements.Add(entityId);
                subscription.SetAvailable(new PlayerClientWriter(world, entity, entityId));
            }
            else
            {
                entitiesNotMatchingRequirements.Add(entityId);
        ...
        public override void Cancel(ISubscription subscription)
        {
            var sub = ((Subscription<PlayerClientWriter>) subscription);
            if (sub.HasValue)
            {
                var reader = sub.Value;
                reader.IsValid = false;
    ...
    public class PlayerClientReader
    {
        public bool IsValid;
        ...
        public PlayerClient.Component Data
        {
            get
            {
                if (!IsValid)
                ...
                return EntityManager.GetComponentData<PlayerClient.Component>(Entity);
            }
        }
        ...
        public Authority Authority
        {
            get
            {
                ...
                return ComponentUpdateSystem.GetAuthority(EntityId, PlayerClient.ComponentId);
            }
        }
        ...
        public event Action<Authority> OnAuthorityUpdate
        {
            add
            {
                ...
                var key = CallbackSystem.RegisterAuthorityCallback(EntityId, PlayerClient.ComponentId, value);
        ...
        private Dictionary<Action<PlayerClient.Update>, ulong> updateCallbackToCallbackKey;
        public event Action<PlayerClient.Update> OnUpdate
        {
            add
            {
                if (updateCallbackToCallbackKey == null)
                {
                    updateCallbackToCallbackKey = new Dictionary<Action<PlayerClient.Update>, ulong>();
                }
        ...
        internal PlayerClientReader(World world, Entity entity, EntityId entityId)
        {
            Entity = entity;
            EntityId = entityId;
    ...
    public class PlayerClientWriter : PlayerClientReader
    {
        ...
        {
            ComponentUpdateSystem.AcknowledgeAuthorityLoss(EntityId, PlayerClient.ComponentId);
        }
    }
}
