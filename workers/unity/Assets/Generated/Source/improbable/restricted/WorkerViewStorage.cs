...
namespace Improbable.Restricted
{
    ...
    {
        public class WorkerViewStorage : IViewStorage, IViewComponentStorage<Snapshot>,
            IViewComponentUpdater<Update>
        {
            ...
            public void ApplyUpdate(long entityId, in Update update)
            {
                ...
                {
                    data.WorkerType = update.WorkerType.Value;
                }

                if (update.Connection.HasValue)
                {
                    data.Connection = update.Connection.Value;
                }
