...
namespace Improbable.Restricted
{
    ...
    {
        public class DisconnectDiffCommandDeserializer : ICommandDiffDeserializer
        {
            ...
            {
                global::Improbable.Restricted.DisconnectResponse? rawResponse = null;
                if (op.StatusCode == StatusCode.Success)
                {
                    rawResponse = global::Improbable.Restricted.DisconnectResponse.Serialization.Deserialize(op.Response.SchemaData.Value.GetObject());
                }

                var commandContext = commandMetaData.GetContext<global::Improbable.Restricted.DisconnectRequest>(ComponentId, 1, op.RequestId);
                commandMetaData.MarkIdForRemoval(ComponentId, 1, op.RequestId);

                var response = new Disconnect.ReceivedResponse(
        ...
        public class DisconnectCommandSerializer : ICommandSerializer
        {
            public uint GetComponentId()
            {
                return ComponentId;
            ...
            {
                var storage = (DisconnectCommandsToSendStorage) messages.GetCommandSendStorage(ComponentId, 1);
                ...
                for (int i = 0; i < requests.Count; ++i)
                {
                    ...
                    var context = new CommandContext<global::Improbable.Restricted.DisconnectRequest>(request.SendingEntity, request.Request.Payload, request.Request.Context, request.RequestId);
                    commandMetaData.AddRequest<global::Improbable.Restricted.DisconnectRequest>(ComponentId, 1, in context);

                    var schemaCommandRequest = new global::Improbable.Worker.CInterop.SchemaCommandRequest(ComponentId, 1);
                    global::Improbable.Restricted.DisconnectRequest.Serialization.Serialize(request.Request.Payload, schemaCommandRequest.GetObject());
                ...
                for (int i = 0; i < responses.Count; ++i)
                {
                    ...
                    global::Improbable.Restricted.DisconnectResponse.Serialization.Serialize(response.Payload.Value, schemaCommandResponse.GetObject());
